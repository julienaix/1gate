...
def on_message(client, userdata, message):
	...
	#If there is no match in MODBUS table
        ...
        #[byte #1-2][byte #3-4][byte #5-6][byte #7-8][byte #9-10][byte #10-11][byte #11-12][byte #13-14][byte #15-16][byte #17-18][byte #19-20][byte #21-22][byte #23-24][byte #25-26][byte #27-28]
        #[------------------------------------devEUI][--------------Timestamp][--------------------------------------------------------------------------------------------------------------------        
	...
	#Then 2 bytes are used to store timestamp 
        builder.add_32bit_uint(stamp_tm)
	#Add the zero bytes to fill the 32 bytes field
        ...
        result  = modbus_client.write_registers(devADRtab[i], content, skip_encode=True, unit=1)
        ...
        for line in devicesLines:
            if(line.split(',')[0] == devEUI_str):
                devices.write(devEUI_str + ',' + devEUI_str + hex(stamp_tm)[2:] + payloadStr + ',' + now_tm + '\n')
            else:
                devices.write(line)
...
def makeTab():
    ...
    for line in devsLines :
        if (line != "\n"):
            ...
            i=i+1
            #Increment modbus register address : 1 device = 22 "boxes" (1 box = 2 bytes i.e. 0xFFFF) 
            j=j+22
...
modbus_client = ModbusClient(host="localhost", port=502, unit_id=1, auto_open=True)
