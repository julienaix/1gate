<html xmlns = "http://www.w3.org/1999/xhtml" xml:lang="fr" lang="fr">
    ...
    <body>
        ...
        //0.Check the state of SSL buttons

	if($_POST['ssl'] == "on")
        {
                echo "MQTT connection is secured (SSL)";
                echo "<br \>";
		//Open configuration file and update ssl configuration
		$file_content=file_get_contents($file_path);
                //Find the ssl line on the file and replace by new value
                $new_content = preg_replace("#mqttSSL=(.*)\n#","mqttSSL=1\n",$file_content);
                //Replace with new content
                file_put_contents($file_path, $new_content);

        }
        elseif($_POST['ssl'] == "off")
        {
	...
	//6.Check the MQTT server topic publish header
        if($_POST['mqtt_pubhead'] != NULL)
        {
                $head = $_POST['mqtt_pubhead'];
                echo "Changed the MQTT server topic publish header to : $head";
                echo "<br \>";
                //Open the configuration file
                $file_content=file_get_contents($file_path);
                //Find the line on the file and replace by new one
                $new_content = preg_replace("#mqttPubHeader=(.*)\n#","mqttPubHeader=$head\n",$file_content);
                //Replace with new content
                file_put_contents($file_path, $new_content);
        }

        //7.Check the MQTT server topic subscribe header
        if($_POST['mqtt_subhead'] != NULL)
        {
                $head = $_POST['mqtt_subhead'];
                echo "Changed the MQTT server topic subscribe header to : $head";
                echo "<br \>";
                //Open the configuration file
                $file_content=file_get_contents($file_path);
                //Find the line on the file and replace by new one
                $new_content = preg_replace("#mqttSubHeader=(.*)\n#","mqttSubHeader=$head\n",$file_content);
                //Replace with new content
                file_put_contents($file_path, $new_content);
        }

	//8.Check the MQTT Cliend Id
        ...
        //9. Check if delete the previous certificates
        if($_POST['del'] == "yes")
        {
                echo "Previous SSL certfificates have been deleted";
                echo "<br \>";
                //Remove previous certificates
                shell_exec('sudo rm /home/ogate/LTEM/certificates/module.key');
                shell_exec('sudo rm /home/ogate/LTEM/certificates/module.cer');
                shell_exec('sudo rm /home/ogate/LTEM/certificates/server.cer');
        }



        //10.Upload the MQTT broker server root CA certificate
	...
	//11.Upload the client (self signed) certificate
	...
	//test if file has been uploaded without errors
        ...
        //Restart application
        echo "LTEM application is restarting...";
        echo "<br \>";
        shell_exec('sudo systemctl restart ltem');
        ?>
